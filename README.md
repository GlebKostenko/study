...
Составьте таблицу с игроками и запишите её в выходной файл ( results.txt - его можно создать кодом на java ). Пртестируйте работу.  
...
Вам дали здание разработать приложение для сети спортивных залов, в этой части вам предстоит реализовать первую часть,
во-первых разберитесь с тем как подключить базу данных, попробуйте для начала что-то сделать из консоли Linux или
desktop-приложения на windows, после этого определите, кто будет фигурировать в базе данных, т.е. о чем и о ком вы
...
Выделите пэкедж с моделями, где будут лежать классы соответствующие таблицам в вашей БД, а также сделайте пэкедж dao,
...
coach с полями id, id_gym, description_of_training, name, surname, age, work_experience, price
client с полями id, id_coach, name, surname, age
```
